...
x_1_dist_sample_0 = (Distribution(x_1_dist_center_0, int(PARAMS["amount_samples"] / 2), device=DEVICE)
                     .with_gaussian_noise(variance=variance_source))

x_1_dist_sample_1 = (Distribution(x_1_dist_center_1, int(PARAMS["amount_samples"] / 2), device=DEVICE)
                     .with_gaussian_noise(variance=variance_source))

x_0_sample = x_0_dist_sample_0.merged_with(x_0_dist_sample_1).tensor
x_1_sample = x_1_dist_sample_0.merged_with(x_1_dist_sample_1).tensor

npe_vanilla = Metrics.calculate_normalized_path_energy(model_vanilla, x_0_sample, x_1_sample)
npe_mac = Metrics.calculate_normalized_path_energy(model_mac, x_0_sample, x_1_sample)
npe_ot_cfm = Metrics.calculate_normalized_path_energy(model_ot_cfm, x_0_sample, x_1_sample)
npe_ref_ot_cfm = Metrics.calculate_normalized_path_energy(model_ref_ot_cfm, x_0_sample, x_1_sample)

print("Vanilla npe: {}".format(npe_vanilla))
print("MAC npe: {}".format(npe_mac))
print("OT-CFM npe: {}".format(npe_ot_cfm))
print("Reflow OT-CFM npe: {}".format(npe_ref_ot_cfm))
