...
# data
x_1_train = load_mnist_tensor(PARAMS["size_train_set"])
x_0_train = torch.randn_like(x_1_train)
...
# model
model = UnetVelocityModel(dropout_rate=PARAMS["dropout_rate_model"])
path = AffineProbPath(CondOTScheduler())
optimizer = torch.optim.Adam(model.parameters(), PARAMS["learning_rate"])
trainer = CondTrainer(model, optimizer, path, PARAMS["num_epochs"])
model_path = os.path.join(MODEL_SAVE_PATH, "model_MNIST_2025-11-04_16-29-55.pth")
...
if SAMPLE_FROM_MODEL:
    model = load_model_unet(model_path, PARAMS["dropout_rate_model"])
    x_0_sample = torch.randn(PARAMS["amount_samples"], *x_0_train.shape[1:], device=x_0_train.device)
