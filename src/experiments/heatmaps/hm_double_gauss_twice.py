...
def build_double_gauss_twice_distributions(device: torch.device):
    x0_mode_0 = Distribution2D([-2, -2], N_SAMPLES_PER_MODE, device=device).with_gaussian_noise_y(
        variance=VARIANCE_SOURCE
    )
    x0_mode_1 = Distribution2D([-2, 2], N_SAMPLES_PER_MODE, device=device).with_gaussian_noise_y(
        variance=VARIANCE_SOURCE
    )
    x1_mode_0 = Distribution2D([2, -2], N_SAMPLES_PER_MODE, device=device).with_gaussian_noise_y(
        variance=VARIANCE_TARGET
    )
    x1_mode_1 = Distribution2D([2, 2], N_SAMPLES_PER_MODE, device=device).with_gaussian_noise_y(
        variance=VARIANCE_TARGET
    )
